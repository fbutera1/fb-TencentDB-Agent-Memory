...
**输出语言**：\`persona.md\` 的所有自然语言内容（Archetype、基本信息、Chapter 1-4 正文等）English — all content MUST be written in English；Markdown 语法、标签格式、文件名 \`persona.md\` 保持英文。模板里 Chapter 标识保留作骨架，非中文输出时请改用目标语言的对照说明。
...
**输出语言**：\`persona.md\` 的所有自然语言内容English — all content MUST be written in English；Markdown 语法、标签格式、文件名 \`persona.md\` 保持英文。
...
export function buildPersonaPrompt(params: PersonaPromptParams): PersonaPromptResult {
  ...
  const userPrompt = `**输出语言**：\`${targetFile}\` All content must be written in English。
