...
**输出语言**：\`.md\` 场景文件的所有自然语言内容（文件名、章节标题、正文）English — all content MUST be written in English；META 字段名（created/updated/summary/heat）和 \`[DELETED]\` 等标记保持英文。模板中给出的中文章节标题（\`## 用户核心特征\` 等）作为结构骨架——非中文输出时请用目标语言的等价表达替换。
...
**输出语言**：\`.md\` 场景文件的所有自然语言内容（文件名、章节标题、正文）English — all content MUST be written in English；META 字段名（created/updated/summary/heat）和 \`[DELETED]\` 等标记保持英文。模板中的中文章节标题仅作为结构骨架，非中文输出时请用目标语言的等价表达替换。
...
export function buildSceneExtractionPrompt(params: SceneExtractionPromptParams): SceneExtractionPromptResult {
  ...
  const userPrompt = `**输出语言**：All scene file content must be written in English。
${warningSection}
